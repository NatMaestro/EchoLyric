    ...
    <div className="max-w-md mx-auto py-16 px-4 space-y-8">
      ...
      >
        ...
        <p className="text-muted-foreground text-pretty">
          Use Google, GitHub, or email. New accounts can{' '}
          <Link href={`/signup?callbackUrl=${encodeURIComponent(callbackUrl)}`} className="text-primary hover:underline">
            sign up here
          </Link>
          — your profile is created the first time you continue with a provider or register with email.
        </p>
      </motion.div>

      {!authSecretOk && (
        <p className="text-sm text-amber-600 dark:text-amber-400 text-pretty text-center">
          Set <code className="text-xs bg-secondary/80 px-1 rounded">AUTH_SECRET</code> in your environment, then
      ...
      {oauthAny && emailSignInAvailable && authSecretOk && (
      ...
      {emailSignInAvailable && authSecretOk ? (
        <motion.form
          initial={{ opacity: 0, y: 12 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        <p className="text-sm text-muted-foreground text-center text-pretty">
          Add Google (<code className="text-xs bg-secondary/80 px-1 rounded">AUTH_GOOGLE_ID</code> /{' '}
          <code className="text-xs bg-secondary/80 px-1 rounded">AUTH_GOOGLE_SECRET</code>), GitHub, or{' '}
          <code className="text-xs bg-secondary/80 px-1 rounded">DATABASE_URL</code> for email sign-in.
        </p>
      ) : null}

      {!emailSignInAvailable && oauthAny && authSecretOk && (
        <p className="text-xs text-muted-foreground text-center text-pretty">
          Email/password needs <code className="bg-secondary/80 px-1 rounded">DATABASE_URL</code>.
        </p>
      )}
